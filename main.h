...
typedef void (*BFS_TopDown_Function)(uint32_t* off, uint32_t* ind, uint32_t* queue, uint32_t* level, uint32_t currRoot);
...
typedef uint32_t (*BFS_BottomUp_Function)(uint32_t* off, uint32_t* ind, uint32_t* bitmap, uint32_t* level, uint32_t currRoot, uint32_t nv, uint32_t currLevel);
